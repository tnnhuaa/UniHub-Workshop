import { AlertTriangle, Calendar, CheckCircle2, MapPin, User, Users, XCircle } from "lucide-react";
...
const getRemainingSeats = (workshop: Pick<WorkshopApiDto, "capacity" | "registeredCount">) =>
  Math.max(workshop.capacity - workshop.registeredCount, 0);

const getStatusMeta = (workshop: WorkshopApiDto): WorkshopCardData["status"] => {
...
export const mapWorkshopToCard = (
  ...
  return {
    ...
    price: {
      label: toNumber(workshop.price) === 0 ? "Free" : `$${toNumber(workshop.price).toFixed(2)}`,
...
export const mapRegistrationToScheduleViewModel = (
  ...
  return {
    ...
    dateLabel,
    timeLabel,
    priceLabel: toNumber(workshop.price) === 0 ? "Free" : `$${toNumber(workshop.price).toFixed(2)}`,
