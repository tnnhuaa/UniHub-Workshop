...
### Cross-cutting libraries (planned: `src/modules/` or `src/libs/`)
...
- **Package manager**: pnpm (monorepo workspace). Do NOT use npm or yarn.
- **pnpm scripts** (from root `package.json`):
  - `pnpm api:dev` — start backend in watch mode
  - `pnpm api:build` — compile TypeScript to JS
